...
DIR="$HOME/Desktop/disney-tracker"
...
check_site() {
  ...
  if echo "$body" | grep -qi "space mountain\|magic kingdom\|wait\|jungle cruise\|haunted mansion"; then
    return 0
  fi
  return 1
}
...
heal() {
  ...
  if grep -q "YOUR_WORKER_URL_HERE" "$DIR/index.html" 2>/dev/null; then
    ...
    sed -i '' "s|YOUR_WORKER_URL_HERE|$WORKER_URL|g" "$DIR/index.html"
  fi
